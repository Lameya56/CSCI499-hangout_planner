    ...
    for (const days of milestones) {
      ...
      const plansResult = await pool.query(
        `SELECT
         ...
         AND p.confirmed_date <  (CURRENT_DATE + INTERVAL '${days + 1} days')`
      );
...
cron.schedule("*/1 * * * *", async () => {
  ...
  try {
    //Only fetch plans whose deadline passed 24+ hours ago
    const planResult = await pool.query(`
      SELECT id, title, host_id, deadline, time, confirmed_date, confirmed_activity_id
      FROM plans
      WHERE status = 'confirmed'
        AND decision_over_email_sent = FALSE
        AND deadline + INTERVAL '24 hours' <= NOW()
      ORDER BY deadline ASC
    `);

    if (planResult.rowCount === 0) {
      console.log("No plans eligible for +24hr deadline emails.");
      return;
    }
    ...
    for (const plan of planResult.rows) {
      ...
      console.log(`Sent decision-over emails to ${participants.rowCount} participants.`);
      ...
      // Mark plan as emailed (prevents re-sending)
      await pool.query(`
        UPDATE plans
